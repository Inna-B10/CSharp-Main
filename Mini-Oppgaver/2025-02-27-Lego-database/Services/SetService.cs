...
{
  public static SetModel? ParseSetLine(string csvLine, int lineNum, string filePath)
  {
    try
    {
      var csvData = csvLine.Split(",");

      //if line does not match SetModel format return null (no error for continue parsing other lines)
      if (csvData.Length < 6)
      {
